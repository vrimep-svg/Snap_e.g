...
public:
  ...
  {
    if (STATUS_FAILED(state.verifyFlatbuffers(VerifyProgramTaskBuffer)))
    {
      return;
    }
    auto myState = UnPackProgramTask(state.getData());
    myState->id = m_id;
    flatbuffers::FlatBufferBuilder builder;
    builder.Finish(ProgramTask::Pack(builder, myState.get()));
    m_state.copyFlatbuffers(builder);
    m_taskState = myState->state;
  }
  ...
  void setError(const Variant &diagnosis)
  {
    if (STATUS_FAILED(diagnosis.verifyFlatbuffers(VerifyDiagnosisBuffer)))
    {
      return;
    }
    m_taskState = ProgramTaskState_failed;
    auto task = UnPackProgramTask(m_state.getData());
    auto diag = UnPackDiagnosis(diagnosis.getData());
    flatbuffers::FlatBufferBuilder builder;
    auto diagOffset = Diagnosis::Pack(builder, diag.get());
    builder.Finish(CreateProgramTaskDirect(builder, task->id.c_str(), m_taskState, 100, diagOffset, nullptr));
    m_state.copyFlatbuffers(builder);
  }
  ...
  virtual std::string getResultNodeName()
  {
    return m_resultNodeName;
  }

  virtual Variant getResult()
  {
    return Variant();
  }

  virtual Variant getResultMetadata()
  {
    Variant metadata;
    MetadataHelper::createMetadata(metadata, NodeClass_Variable, true, false, false, false, false, {}, "Result of the task.");
    return metadata;
  }
...
public:
  ...
  void onCreate(const std::string& address, const comm::datalayer::Variant* parameters, const comm::datalayer::IProviderNode::ResponseCallback& callback) override
  {
    ...
    if (task->getResultNodeName().length())
    {
      m_provider->registerNode(m_baseAddress + "/" + myID + "/" + task->getResultNodeName(), this);
    }
  ...
  void onRemove(const std::string& address, const ResponseCallback& callback) override
  {
    ...
    if (part.length())
    {
      ...
      if (parts.size() == 1)
      {
        ...
        m_provider->unregisterNode(m_baseAddress + "/" + task->first + "/" + task->second->getResultNodeName());
  ...
  void onRead(const std::string& address, const comm::datalayer::Variant* , const ResponseCallback& callback) override
  {
    ...
    if (part.length())
    {
      ...
      if (parts.size() == 2)
      {
        ...
        if (parts[1] == task->second->getResultNodeName())
        {
          auto result = task->second->getResult();
          callback(DlResult::DL_OK, &result);
          return;
        }
      }
      callback(DlResult::DL_INVALID_ADDRESS, nullptr);
  ...
  void onMetadata(const std::string& address, const ResponseCallback& callback) override
  {
    ...
    if (part.length())
    {
      ...
      { // subnode of task
        ...
        if (parts[1] == task->second->getResultNodeName())
        {
          auto result = task->second->getResultMetadata();
          callback(DlResult::DL_OK, &result);
          return;
        }
      }
...
protected:
  ...
  typedef std::map<std::string, Task*> TMapTasks;
  TMapTasks m_mapTasks;
