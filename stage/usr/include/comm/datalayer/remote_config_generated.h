...
struct RemoteConfigT : public ::flatbuffers::NativeTable {
  ...
  RemoteConfigT() = default;
  RemoteConfigT(const RemoteConfigT &o);
  RemoteConfigT(RemoteConfigT&&) FLATBUFFERS_NOEXCEPT = default;
  RemoteConfigT &operator=(RemoteConfigT o) FLATBUFFERS_NOEXCEPT;
};
...
inline RemoteConfigT::RemoteConfigT(const RemoteConfigT &o) {
  remotes.reserve(o.remotes.size());
  for (const auto &remotes_ : o.remotes) { remotes.emplace_back((remotes_) ? new comm::datalayer::remote::ConfigItemT(*remotes_) : nullptr); }
}

inline RemoteConfigT &RemoteConfigT::operator=(RemoteConfigT o) FLATBUFFERS_NOEXCEPT {
  std::swap(remotes, o.remotes);
  return *this;
}
...
inline void RemoteConfig::UnPackTo(RemoteConfigT *_o, const ::flatbuffers::resolver_function_t *_resolver) const {
  (void)_o;
  (void)_resolver;
  { auto _e = remotes(); if (_e) { _o->remotes.resize(_e->size()); for (::flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { if(_o->remotes[_i]) { _e->Get(_i)->UnPackTo(_o->remotes[_i].get(), _resolver); } else { _o->remotes[_i] = std::unique_ptr<comm::datalayer::remote::ConfigItemT>(_e->Get(_i)->UnPack(_resolver)); }; } } else { _o->remotes.resize(0); } }
}
...
inline comm::datalayer::remote::RemoteConfig *GetMutableSizePrefixedRemoteConfig(void *buf) {
  return ::flatbuffers::GetMutableSizePrefixedRoot<comm::datalayer::remote::RemoteConfig>(buf);
}
