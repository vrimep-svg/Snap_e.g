...
public:
  ...
  //! Checks a base directory against a new directory for compatible bfbs schema files
  //! @param[in] baseDir      Path to location of base bfbs files. Location can have subdirectories
  //! @param[in] newDir       Path to location of new bfbs files. Location can have subdirectories
  //! @return none
  void checkCompatible(std::filesystem::path baseDir, std::filesystem::path newDir);
  ...
  void setSetting(Setting setting, uint32_t value);

protected:
private:
  void checkCompatibleInt(std::filesystem::path baseDir, std::filesystem::path newDir);
