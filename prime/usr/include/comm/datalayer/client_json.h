...
public:
  ...
  //! @param[in]  token       Security access &token for authentication as JWT payload (optional)
  //! @return result status of the function
  DlResult writeJsonSync(const std::string& addr, const std::string& json, std::string* error = nullptr, [[maybe_unused]] int32_t indentStep = 0, const std::string& token = "")
  {
    DlResult result;
    Variant typeIn;
    Variant data;
  ...
  //! @param[in]  token       Security access &token for authentication as JWT payload (optional)
  //! @return result status of the function
  DlResult createJsonSync(const std::string& addr, const std::string& json, std::string* error = nullptr, [[maybe_unused]] int32_t indentStep = 0, const std::string& token = "")
  {
    DlResult result;
    Variant type;
    Variant data;
  ...
  DlResult ParseJson(const std::string &addr, const std::string &json, Variant &data, Variant &type, const std::string &reference, std::string *error, const std::string & token)
  {
    ...
    if (STATUS_FAILED(result = m_converter.parseJson(json, &data, error)))
    {
      ...
      if (STATUS_FAILED(result = m_converter.parseJson(json, type, &data, error)))
      {
        return result;
      }
    }
