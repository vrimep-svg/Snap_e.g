...
#define IDATALAYER_CONSUMER_RANGE "[" IDATALAYER_VERSION_MAJOR_STR ".0.0,3.0.0)"                      //! Version range for users of Datalayer
...
#define IDATALAYER_FACTORY_INTERFACE_NAME3 "com.boschrexroth.comm.datalayer.factory3"             //! Interface for the factory which implements version 3 of the datalayer factory. Version 2 of the interface introduces bulk requests.
#define IDATALAYER_FACTORY_INTERFACE_NAME4 "com.boschrexroth.comm.datalayer.factory4"             //! Interface for the factory which implements version 4 of the datalayer factory. Version 2 of the interface introduces events.
#define IDATALAYER_FACTORY_INTERFACE_NAME5 "com.boschrexroth.comm.datalayer.factory5"             //! Interface for the factory which implements version 5 of the datalayer factory. Version 2 of the interface introduces support of bulk for providers.
...
enum class TimeoutSetting
{
  ...
  ZmqHWMSnd,    // High water mark of ZMQ
  ZmqHWMRcv,    // High water mark of ZMQ
};
...
public:
  ...
  virtual DlResult getConnectStatus() = 0;
...
public:
  ...
  virtual void onBulkRead(const comm::datalayer::VecBulkRequest& request, BulkCallback& callback) = 0;
  virtual void onBulkWrite(const comm::datalayer::VecBulkRequest& request, BulkCallback& callback) = 0;
  virtual void onBulkCreate(const comm::datalayer::VecBulkRequest& request, BulkCallback& callback) = 0;
  virtual void onBulkRemove(const comm::datalayer::VecBulkRequest& request, BulkCallback& callback) = 0;
  virtual void onBulkBrowse(const comm::datalayer::VecBulkRequest& request, BulkCallback& callback) = 0;
  virtual void onBulkMetadata(const comm::datalayer::VecBulkRequest& request, BulkCallback& callback) = 0;
};
...
public:
  ...
  //! Register a type to the datalayer
  //! @param[in]  address   Address of the type to register (no wildcards allowed)
  //! @param[in]  type      Variant with flatbuffer type
  //! @result status of function call
  virtual DlResult registerType(const std::string& address, const comm::datalayer::Variant& type) = 0;

  //! Get the variant of a registered type
  //! @param[in]  address   Address of the type to get type (no wildcards allowed)
  //! @param[out]  type     Variant to stored type
  //! @result status of function call
  virtual DlResult getRegisteredType(const std::string& address, comm::datalayer::Variant& type) = 0;
...
public:
  ...
  //! @result status of function call
  virtual DlResult getRegisteredNodePaths(comm::datalayer::Variant& data) = 0;
};

class IProvider3 : public IProvider2
{
public:
  virtual ~IProvider3() {};

  //! Publishes an event
  //! @param data        The payload data of the event. Has to match the type, that is given in the notifyInfo.
  //! @param notifyInfo  Contains additional info about the event with type event_info.fbs
  //! @result status of function call
  virtual DlResult publishEvent(const comm::datalayer::Variant& data, const comm::datalayer::Variant& eventInfo) = 0;
};

class IProvider4 : public IProvider3
{
public:
  virtual ~IProvider4() {};

  //! Adds bulk interface to already registered node address
  //! bulk requests will be forwarded to this interface
  //! @param[in]  address   Address of the node to register (wild cards allowed)
  //! @param[in]  bulk      Bulk Interface
  //! @result status of function call
  virtual DlResult addBulkInterface(const std::string& address, IBulkNode* bulk) = 0;
  ...
  //! @param[in]  timeoutMS Timeout in milliseconds for this node
  //! @result status of function call
  using IProvider::setTimeoutNode;
  virtual DlResult setTimeoutNode(IBulkNode* node, uint32_t timeoutMS) = 0;
...
public:
  ...
  virtual DlResult createMemoryAsync(const std::string& addr, IProvider* provider, size_t sizeBytes, MemoryType type, Variant& options, ResponseCallbackOwner& callback) = 0;
  virtual DlResult createMemorySync(std::shared_ptr<IMemoryOwner>& owner, const std::string& addr, IProvider* provider, size_t sizeBytes, MemoryType type, Variant& options) = 0;
...
class IDataLayerFactory4 : public IDataLayerFactory3
{
public:
  virtual ~IDataLayerFactory4()
  {};

  //! Creates a provider to provide data to the datalayer
  //! @param[in] remote     Remote address of the data layer
  //! @return Provider
  virtual IProvider3* createProvider3(const std::string& remote = DL_INPROC_BACKEND_SERVER) = 0;

};

class IDataLayerFactory5 : public IDataLayerFactory4
{
public:
  virtual ~IDataLayerFactory5()
  {};

  //! Creates a provider to provide data to the datalayer
  //! @param[in] remote     Remote address of the data layer
  //! @return Provider
  virtual IProvider4* createProvider4(const std::string& remote = DL_INPROC_BACKEND_SERVER) = 0;
...
#define IDATALAYER_C_CONSUMER_RANGE_1_9_0 "[1.9.0,2.0.0)"
  ...
  //! Runs the flatc compiler with given compiler options
  //! @param[in]      options   Compiler options for flatc as string array
  //! @param[out]     output    Compiler results (errors and warnings) as string array
  //! @result status of function call
  DLR_RESULT(*flatc)(comm::datalayer::Variant& options, comm::datalayer::Variant& output);

#define IDATALAYER_C_CONSUMER_RANGE_1_9_3 "[1.9.3,2.0.0)"

  //! Returns rejected node paths
  //! @param[in]      provider       Reference to the provider
  //! @param[out]     data           Variant to put list of rejected nodes to
  //! @result status of function call
  DLR_RESULT(*getRejectedNodePaths)(void* provider, comm::datalayer::Variant& data);

#define IDATALAYER_C_CONSUMER_RANGE_2_3_1 "[2.3.1,3.0.0)"
  ...
  //! @result status of function call
  DLR_RESULT(*publishEvent)(void* provider, const comm::datalayer::Variant& data, const comm::datalayer::Variant& eventInfo);

#define IDATALAYER_C_CONSUMER_RANGE_2_5_2 "[2.5.2,3.0.0)"

  //! getDataFromFlatbuffers
  //! @param[in] object      Data of the flatbuffer
  //! @param[in] schema      Schema of the object
  //! @param[in] query       Query string
  //! @param[out] result     Result variant returning the value or the error flatbuffer
  //! @result status of function call
  DLR_RESULT(*getDataFromFlatbuffers)(const comm::datalayer::Variant& object, const comm::datalayer::Variant& schema, const std::string& query, comm::datalayer::Variant& result);

#define IDATALAYER_C_CONSUMER_RANGE_2_7_0 "[2.7.0,3.0.0)"
