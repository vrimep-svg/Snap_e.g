...
struct WatchlistsT : public ::flatbuffers::NativeTable {
  ...
  WatchlistsT() = default;
  WatchlistsT(const WatchlistsT &o);
  WatchlistsT(WatchlistsT&&) FLATBUFFERS_NOEXCEPT = default;
  WatchlistsT &operator=(WatchlistsT o) FLATBUFFERS_NOEXCEPT;
};
...
inline WatchlistsT::WatchlistsT(const WatchlistsT &o) {
  watchlists.reserve(o.watchlists.size());
  for (const auto &watchlists_ : o.watchlists) { watchlists.emplace_back((watchlists_) ? new comm::datalayer::WatchlistT(*watchlists_) : nullptr); }
}

inline WatchlistsT &WatchlistsT::operator=(WatchlistsT o) FLATBUFFERS_NOEXCEPT {
  std::swap(watchlists, o.watchlists);
  return *this;
}
...
inline void Watchlists::UnPackTo(WatchlistsT *_o, const ::flatbuffers::resolver_function_t *_resolver) const {
  (void)_o;
  (void)_resolver;
  { auto _e = watchlists(); if (_e) { _o->watchlists.resize(_e->size()); for (::flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { if(_o->watchlists[_i]) { _e->Get(_i)->UnPackTo(_o->watchlists[_i].get(), _resolver); } else { _o->watchlists[_i] = std::unique_ptr<comm::datalayer::WatchlistT>(_e->Get(_i)->UnPack(_resolver)); }; } } else { _o->watchlists.resize(0); } }
}
...
inline comm::datalayer::Watchlists *GetMutableSizePrefixedWatchlists(void *buf) {
  return ::flatbuffers::GetMutableSizePrefixedRoot<comm::datalayer::Watchlists>(buf);
}
