...
struct Claim FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  bool KeyCompareLessThan(const Claim * const o) const {
    return *claim() < *o->claim();
  }
  int KeyCompareWithValue(const char *_claim) const {
    return strcmp(claim()->c_str(), _claim);
  }
...
struct TokenT : public ::flatbuffers::NativeTable {
  ...
  TokenT() = default;
  TokenT(const TokenT &o);
  TokenT(TokenT&&) FLATBUFFERS_NOEXCEPT = default;
  TokenT &operator=(TokenT o) FLATBUFFERS_NOEXCEPT;
};
...
struct Token FLATBUFFERS_FINAL_CLASS : private ::flatbuffers::Table {
  ...
  bool mutate_iat(uint64_t _iat = 0) {
  ...
  bool mutate_plchandle(uint64_t _plchandle = 0) {
  ...
  bool Verify(::flatbuffers::Verifier &verifier) const {
    return VerifyTableStart(verifier) &&
           ...
           VerifyField<uint64_t>(verifier, VT_IAT, 8) &&
           VerifyField<uint64_t>(verifier, VT_EXP, 8) &&
           VerifyOffset(verifier, VT_NAME) &&
           verifier.VerifyString(name()) &&
           VerifyField<uint64_t>(verifier, VT_PLCHANDLE, 8) &&
...
inline TokenT::TokenT(const TokenT &o)
      : id(o.id),
        iat(o.iat),
        exp(o.exp),
        name(o.name),
        plchandle(o.plchandle),
        scope(o.scope) {
  claims.reserve(o.claims.size());
  for (const auto &claims_ : o.claims) { claims.emplace_back((claims_) ? new comm::datalayer::ClaimT(*claims_) : nullptr); }
}

inline TokenT &TokenT::operator=(TokenT o) FLATBUFFERS_NOEXCEPT {
  std::swap(claims, o.claims);
  std::swap(id, o.id);
  std::swap(iat, o.iat);
  std::swap(exp, o.exp);
  std::swap(name, o.name);
  std::swap(plchandle, o.plchandle);
...
inline void Token::UnPackTo(TokenT *_o, const ::flatbuffers::resolver_function_t *_resolver) const {
  (void)_o;
  (void)_resolver;
  { auto _e = claims(); if (_e) { _o->claims.resize(_e->size()); for (::flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { if(_o->claims[_i]) { _e->Get(_i)->UnPackTo(_o->claims[_i].get(), _resolver); } else { _o->claims[_i] = std::unique_ptr<comm::datalayer::ClaimT>(_e->Get(_i)->UnPack(_resolver)); }; } } else { _o->claims.resize(0); } }
  { auto _e = id(); if (_e) _o->id = _e->str(); }
  { auto _e = iat(); _o->iat = _e; }
  { auto _e = exp(); _o->exp = _e; }
  { auto _e = name(); if (_e) _o->name = _e->str(); }
  { auto _e = plchandle(); _o->plchandle = _e; }
  { auto _e = scope(); if (_e) { _o->scope.resize(_e->size()); for (::flatbuffers::uoffset_t _i = 0; _i < _e->size(); _i++) { _o->scope[_i] = _e->Get(_i)->str(); } } else { _o->scope.resize(0); } }
}
...
inline comm::datalayer::Token *GetMutableSizePrefixedToken(void *buf) {
  return ::flatbuffers::GetMutableSizePrefixedRoot<comm::datalayer::Token>(buf);
}
