...
public:
  ...
  {
    return setSharedPointer(getVariantTypeID<T*>(), reinterpret_cast<uint8_t*>(arrayValue), count * getDataLength<T>(*arrayValue));
  }
  ...
  DlResult serialize(uint8_t* buffer, size_t size) const
  {
    ...
    *reinterpret_cast<VariantType*>(buffer) = m_type;
    ...
    if (isBufferType(m_type))
    {
      memcpy(buffer, m_value.buffer.buffer, std::min<size_t>(m_value.buffer.size, size));
    }
    else
    {
  ...
  DlResult deserialize(uint8_t* buffer, size_t size)
  {
    ...
    if (newType > VariantType::EMPTY || newType < VariantType::UNKNOWN) {
      return DlResult::DL_TYPE_MISMATCH;
    }
    ...
    {
      m_value.ui64 = 0;
      newSize = std::min<size_t>(newSize, sizeof(m_value.ui64));
  ...
  DlResult deserializeShared(uint8_t* buffer, size_t size)
  {
    ...
    else
    {
      setType(newType);
      m_value.ui64 = 0;
      memcpy(&m_value.ui64, buffer, std::min<size_t>(sizeof(m_value.ui64),newSize));
    }
    return DlResult::DL_OK;
  }
...
private:
  ...
  {
    if (m_type == VariantType::STRING && m_value.buffer.size) {
      m_value.buffer.buffer[m_value.buffer.size - 1] = 0;
      return;
    }
