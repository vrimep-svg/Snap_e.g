...
#undef DL_ENUM_VALUE
  ...
  //! Set an raw value
  //! @param[in] variant     Reference to variant
  //! @param[in] value       Pointer to raw data
  //! @param[in] count       Size of raw data
  //! @return Status of set value
  DLR_RESULT DLR_variantSetRaw(DLR_VARIANT variant, const int8_t* value, const size_t size);
  ...
  //! Returns the array of timestamp if the type is a array of timestamp otherwise null
  //! @param[in] variant     Reference to variant
  //! @return Value of the variant
  const uint64_t* DLR_variantGetArrayOfTIMESTAMP(DLR_VARIANT variant);
  ...
  //! @param[in] object      Data of the flatbuffers
  //! @param[in] schema      Schema of the object
  //! @param[in] query       Query string
  //! @param[out] result     Result variant returning the value or the error flatbuffers
  //! @result status of function call
  DLR_RESULT DLR_getDataFromFlatbuffers(const DLR_VARIANT object, const DLR_VARIANT schema, const char* query, DLR_VARIANT result);
