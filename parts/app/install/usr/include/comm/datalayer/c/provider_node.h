...
extern "C" {
  ...
  typedef size_t DLR_PROVIDER_NODE_CALLBACKDATA2;
  ...
  //! @param[in] result            Status result of the request
  //! @param[in] data              Result data of the request
  typedef void(*DLR_PROVIDER_NODE_CALLBACK2)(DLR_PROVIDER_NODE_CALLBACKDATA2 callbackdata, DLR_RESULT result, const DLR_VARIANT data);
  ...
  //! @param[in] callbackdata      Callback data to give with callback function
  //! @return Result will be ignored
  typedef DLR_RESULT(*DLR_PROVIDER_NODE_FUNCTION2) (DLR_PROVIDER_NODE_CALLBACKDATA2 userData, const char* address, DLR_PROVIDER_NODE_CALLBACK2 callback, DLR_PROVIDER_NODE_CALLBACKDATA2 callbackdata);
  ...
  //! @param[in] callbackdata      Callback data to give with callback function
  //! @return Result will be ignored
  typedef DLR_RESULT(*DLR_PROVIDER_NODE_FUNCTION_DATA2)(DLR_PROVIDER_NODE_CALLBACKDATA2 userData, const char* address, DLR_VARIANT data, DLR_PROVIDER_NODE_CALLBACK2 callback, DLR_PROVIDER_NODE_CALLBACKDATA2 callbackdata);
  ...
  //! Type to define callback functions for a node
  typedef struct DLR_PROVIDER_NODE_CALLBACKS2
  {
    DLR_PROVIDER_NODE_CALLBACKDATA2 userData;         // userData to identify the node - this user data will be given with all functions below as userData
    DLR_PROVIDER_NODE_FUNCTION_DATA2    onCreate;     // function to call for a create request
    DLR_PROVIDER_NODE_FUNCTION2         onRemove;     // function to call for a remove request
    DLR_PROVIDER_NODE_FUNCTION2         onBrowse;     // function to call for a browse request
    DLR_PROVIDER_NODE_FUNCTION_DATA2    onRead;       // function to call for a read request
    DLR_PROVIDER_NODE_FUNCTION_DATA2    onWrite;      // function to call for a write request
    DLR_PROVIDER_NODE_FUNCTION2         onMetadata;   // function to call for a metadata request
    DLR_PROVIDER_SUBSCRIPTION_FUNCTION onSubscribe;   // function to call for a subscription request - nullptr if not supported
    DLR_PROVIDER_SUBSCRIPTION_FUNCTION onUnsubscribe; // function to call for a unsubscription request - nullptr if not supported
  }DLR_PROVIDER_NODE_CALLBACKS2;
  ...
  //! Creates a node.
  //! Use this function if you language checks void* in a c function call as own objects (eg. GoLang)
  //! @param[in] callbacks    Callbacks for the node
  //! @return Created node
  DLR_PROVIDER_NODE DLR_providerNodeCreate2(DLR_PROVIDER_NODE_CALLBACKS2 callbacks);
